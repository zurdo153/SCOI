...
public class Cat_Tipos_De_Equipo extends JFrame{
	...
	public void init_tablaf(){
    	...
    	this.tabla.getColumnModel().getColumn(2).setMinWidth(75);
    	this.tabla.getColumnModel().getColumn(2).setMinWidth(55);
    	
		String comandof="select folio,nombre,serie,case when status='V' then 'Vigente' else 'Cancelado' end as estatus from  tb_tipos_de_equipo order by nombre";
	...
	public DefaultTableModel modelo = new DefaultTableModel(null, new String[]{"Folio","Tipos","Serie","Estatus"}){
		 @SuppressWarnings("rawtypes")
			Class[] types = base();
	...
	JTextField txtTipos = new Componentes().text(new JCTextField(), "Tipos", 100, "String");
	JTextField txtSerie   = new Componentes().text(new JCTextField(), "Serie", 100, "String");
	...
	public Cat_Tipos_De_Equipo(){
		...
		this.panel.setBorder(BorderFactory.createTitledBorder("Tipos De Equipo"));
		this.setTitle("Tipos De Equipos Para Activos");
		...
		this.menu_toolbar.setFloatable(false);
		 ...
		 int x=15, y=20,width=120,height=20,sep=30;
		this.panel.add(menu_toolbar).setBounds               (x     ,y      ,width*4-60 ,height );
		this.panel.add(new JLabel("Folio:")).setBounds       (x     ,y+=40  ,width      ,height );
		this.panel.add(txtFolio).setBounds                   (x+=sep,y      ,width-30   ,height );
		this.panel.add(new JLabel("Serie:")).setBounds       (x+=100,y     ,width       ,height );
		this.panel.add(txtSerie).setBounds                   (x+=sep,y      ,width-20   ,height );
		
		this.panel.add(new JLabel("Estatus:")).setBounds     (x+=110,y      ,width      ,height );
		this.panel.add(cmb_status).setBounds                 (x+=40 ,y      ,width-20   ,height );
		 x=15;width=120;
		this.panel.add(new JLabel("Tipos:")).setBounds       (x     ,y+=30  ,width      ,height );
		this.panel.add(txtTipos).setBounds                   (x+=sep,y      ,width+260  ,height );
         width=410;
        ...
        tbl.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
	        	if(e.getClickCount()==1){
					...
					txtTipos.setText(tabla.getValueAt(fila,1)+"");
					txtSerie.setText(tabla.getValueAt(fila,2)+"");
	...
	public String Valida(){
		...
		String Tipos = txtTipos.getText().trim();
	    String Mensaje ="";
	    if(txtTipos.getText().equals("")){Mensaje+="\nEs Requerido Para Guardar Que Se Alimente Un Tipo De Equipo"; }
	    if(txtSerie.getText().equals("")){Mensaje+="\nEs Requerido Para Guardar Que Se Alimente Una Serie De Equipo"; }
		for(int i =0; i<tabla.getRowCount(); i++){
			    	  if(!Folio.equals(tabla.getValueAt(i, 0).toString().trim()))  {   	
			    		  if(Tipos.equals(tabla.getValueAt(i, 1).toString().trim())){	Mensaje+="\nEl Tipo De Equipo :"+Tipos+" Existe En El Folio:"+tabla.getValueAt(i, 0).toString().trim();	    	  };
			    	  };
        }
		return Mensaje;
	}	
	...
	ActionListener guardar = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			Obj_Tipos_De_Equipos Tipos = new Obj_Tipos_De_Equipos();
			...
			if(!Mensaje.equals("")){
				...
				Tipos.setFolio(Integer.valueOf(txtFolio.getText()));
				Tipos.setTipos_De_Equipo(txtTipos.getText().toString().toUpperCase());
				Tipos.setSerie(txtSerie.getText().toString().toUpperCase());
				Tipos.setEstatus(cmb_status.getSelectedItem().toString());
				Tipos.setGuardaModifica(NuevoModifica);
				if(Tipos.guardar()){
	...
	public static void main(String args[]){
		try{UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
			new Cat_Tipos_De_Equipo().setVisible(true);
