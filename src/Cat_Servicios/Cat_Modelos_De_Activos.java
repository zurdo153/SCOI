...
public class Cat_Modelos_De_Activos extends JFrame{
	...
	public void init_tablaf(){
		...
		String comandof="select folio,nombre,case when status='V' then 'Vigente' else 'Cancelado' end as estatus from  tb_modelo_de_equipo order by nombre";
	...
	public DefaultTableModel modelo = new DefaultTableModel(null, new String[]{"Folio","Modelos","Estatus"}){
		 @SuppressWarnings("rawtypes")
			Class[] types = base();
	...
	JTextField txtModelos = new Componentes().text(new JCTextField(), "Modelos", 100, "String");
	...
	public Cat_Modelos_De_Activos(){
		...
		this.panel.setBorder(BorderFactory.createTitledBorder("Modeloss"));
		this.setTitle("Modeloss");
		...
		this.panel.add(cmb_status).setBounds                 (x+=70 ,y      ,width      ,height );
		 x=15;width=120;
		this.panel.add(new JLabel("Modelos:")).setBounds       (x     ,y+=30  ,width      ,height );
		this.panel.add(txtModelos).setBounds                   (x+=sep,y      ,width*3    ,height );
         width=410;
	...
	public void panelEnabledTrue(){	
		txtModelos.setEditable(true );
		btnEditar.setEnabled  (false);
		btnNuevo.setEnabled   (false);
		cmb_status.setEnabled(true);
		this.addWindowListener(new WindowAdapter() {
            public void windowOpened( WindowEvent e ){
             txtModelos.requestFocus();
           }
        });
	}
        ...
        tbl.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
	        	if(e.getClickCount()==1){
					...
					txtModelos.setText(tabla.getValueAt(fila,1)+"");
	...
	public String Valida(){
	    ...
	    String Mensaje ="";
	    if(txtModelos.getText().equals("")){Mensaje+="\nEs Requerido Para Guardar Que Se Alimente Un Modelos"; }
		for(int i =0; i<tabla.getRowCount(); i++){
			    	  if(!Folio.equals(tabla.getValueAt(i, 0).toString().trim()))  {   	
			    		  if(Modelos.equals(tabla.getValueAt(i, 1).toString().trim())){	Mensaje+="\nEl Modelos :"+Modelos+" Existe En El Folio:"+tabla.getValueAt(i, 0).toString().trim();	    	  };
			    	  };
        }
		return Mensaje;
	}	
	...
	ActionListener guardar = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			Obj_Modelos_De_Activos Modelos = new Obj_Modelos_De_Activos();
			...
			if(!Mensaje.equals("")){
				...
				Modelos.setFolio(Integer.valueOf(txtFolio.getText()));
				Modelos.setModelo_De_Activo(txtModelos.getText().toString().toUpperCase());
				Modelos.setEstatus(cmb_status.getSelectedItem().toString());
				Modelos.setGuardaModifica(NuevoModifica);
				if(Modelos.guardar()){
	...
	public static void main(String args[]){
		try{UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
			new Cat_Modelos_De_Activos().setVisible(true);
