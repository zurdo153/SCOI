...
public class Cat_Aspectos extends JFrame{
	...
	public void init_tablaf(){
		...
		String comandof="select folio,aspecto,ponderacion,orden,case when estatus='V' then 'Vigente' else 'Cancelado' end as estatus from cuadrantes_aspecto order by orden";
	...
	public DefaultTableModel modelo = new DefaultTableModel(null, new String[]{"Folio","Aspecto","Ponderacion","Orden","Estatus"}){
		 @SuppressWarnings("rawtypes")
			Class[] types = base();
	...
	JTextField txtAspecto = new Componentes().text(new JCTextField(), "Aspecto", 100, "String");
	...
	public Cat_Aspectos(){
		...
		this.panel.setBorder(BorderFactory.createTitledBorder("Aspectos"));
		...
		this.panel.add(cmb_status).setBounds                 (x+=70 ,y      ,width      ,height );
		 x=15;width=120;
		this.panel.add(new JLabel("Aspecto:")).setBounds     (x     ,y+=30  ,width      ,height );
		this.panel.add(txtAspecto).setBounds                 (x+=sep,y      ,width*3    ,height );
		 x=15;
        ...
        tbl.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
	        	if(e.getClickCount()==1){
					...
					txtAspecto.setText(tabla.getValueAt(fila,1)+"");
	...
	ActionListener guardar = new ActionListener(){
		public void actionPerformed(ActionEvent e){
			...
			if(!Mensaje.equals("")){
				...
				aspecto.setAspecto(txtAspecto.getText().toString());
				aspecto.setOrden(Integer.valueOf(txtOrden.getText()));
				aspecto.setPonderacion(Integer.valueOf(txtValor.getText()));
	...
	public static void main(String args[]){
		try{UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
			new Cat_Aspectos().setVisible(true);
